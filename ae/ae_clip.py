# Encoder is pretrained on ImageNet classification and is frozen
...
class EarlyResNet(torch.nn.Module):
    # modified_resnet block3 = 12th module
    def __init__(self, full_model, last_module_idx=12):
        super().__init__()
        early_mod = []
...
class AE(nn.Module):
    def __init__(
        ...
        self.alpha = alpha
        self.lr = lr
        self.is_enc_half = False
        # Encoder architecture: Modified ResNet50 from CLIP
        clip_rn50, preprocess = clip.load("RN50")
        for param in clip_rn50.parameters():
            param.data = param.data.float()
            # print(param.dtype)
            # if isinstance(param.dtype, torch.float16):
            #     self.is_enc_half = True
        full_resnet_50 = clip_rn50.visual
...
if __name__ == "__main__":
    ...
    dec_channel_config_str = "256:64,128:64,64:64,32:128,16:128,8:256,4:512,1:1024"
