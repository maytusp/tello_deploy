...
def resnet18(in_channels, base_planes, ngroups):
    model = ResNetEarlyLayers(in_channels, base_planes, ngroups, BasicBlock, [2, 2, 2, 2])

    return model


def resnet50(in_channels: int, base_planes: int, ngroups: int) -> ResNetEarlyLayers:
    model = ResNetEarlyLayers(in_channels, base_planes, ngroups, Bottleneck, [3, 4, 6, 3])
